...
public class TFIDFRecommenderService {
    ...
    public List<CareerItem> recommendByUserProfile(Long id) {
        ...
        if (user.getCareer() != null && !user.getCareer().isEmpty()) {
            ...
            if (tags == null || tags.isEmpty()) {
                ...
                for (String name : generateTagsFromCareer(user.getCareer())) {
                    Tag tag = Tag.builder().tagName(name).userProfile(user).build();
                    user.addTag(tag);
                    log.debug("➕ 태그 생성됨: {}", name);
                }
                userRepo.save(user);
                tags = user.getTags();
    ...
    public List<String> generateTagsFromCareer(String career) {
    ...
    public List<String> translateCareerToEnglishTags(String koreanText) {
        ...
        String prompt = "진로: " + koreanText +
                "\n아래 리스트에 있는 태그들 중에서만 해당 진로와 관련 있는 키워드 5개를 추출해줘. " +
                "모든 키워드는 소문자로 출력하고, 예시는 반드시 아래 형식처럼 출력해줘. 다른 설명은 하지 마.\n" +
                "태그 리스트: [\"backend\", \"java\", \"spring\", \"sql\", \"rest api\", \"docker\", \"aws\", \"linux\", \"mysql\", " +
                "\"devops\", \"research\", \"lab\", \"data science\", \"statistics\", \"python\", \"pandas\", \"tensorflow\", \"pytorch\", " +
                "\"law\", \"government\", \"public service\", \"tax\", \"policy\", \"teacher\", \"judge\", \"youtube\", \"influencer\", " +
                "\"content\", \"editing\", \"vlog\", \"video\", \"social media\", \"entrepreneurship\", \"freelancer\", \"marketing\", " +
                "\"accounting\", \"startup\", \"sales\", \"finance\", \"celebrity\", \"entertainment\", \"performer\"]\n" +
                "예시 형식: [\"backend\", \"java\", \"mysql\", \"devops\", \"linux\"]";
    ...
    public void updateUserProfile(Long userId, List<String> careers) {
        ...
        List<String> tagNames = generateTagsFromCareer(combinedCareer);
        List<CareerItem> recommendedItems = recommendByKeywords(tagNames);
        ...
        newTags.forEach(tag ->
                log.debug("➕ 태그 추가: {} (CareerItem ID: {})",
                        tag.getTagName(),
                        tag.getCareerItem() != null ? tag.getCareerItem().getId() : "없음"));
